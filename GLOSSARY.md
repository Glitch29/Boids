...
forward-navigates to `Y` within `P`, and **whose projection to `(x, y)` is diagonally connected**.
Rests on adjacent headings' steps differing by at most one pixel, which they do at radius 40.
What it is for, philosophically: the shortest way onto `P` from off it is roughly the continuous
trajectory — no detour round a missing phase, no lockout where phase does not bleed. Does not fix
`P`; built by `PhasePath.coverConnected`, measured by the **join field**. `EDGES.md` §2a
"Phase-complete paths". Not the same thing as `DecisionZone.subpath`'s chain of `S_k`, which is
the stop-gap it replaces. The first definition (2026-09-14) also constrained the **funnels** and
failed on the lattice, not the path; kept as a record.

**lane** — the pixels a canonical path `S` sweeps: its own and, per step, the samples the step
passes through, less dead pixels. What the other phases of the step lattice stand on.
`PhasePath.lane`.
...
of `|Δ|` between 4-adjacent pixels at the same heading: **at most 1 everywhere** for a
...
**phase ceiling / the loop clamps phase** — what the shortest-lap table shows: in 260 ticks a
boid at phase `p` on the straight can land at any phase from `p` up to `x ≡ 1 (mod 4)` in the
order `2, 3, 0, 1`, never past it, and reaching a phase past the ceiling costs a full tick. Phase
1 is a fixed point. So the sub-tick phase is not carried round the loop, and a quarter-tick tau
cannot be either. `EDGES.md` §5.

**starting line** — a column `x₀` across a straight, taken as the boundary between pixel
columns `x₀−1` and `x₀`: starts at `x₀+k` are `k` quarter-ticks past it, finishes at `x₀−1−k`
...
**`F/4` is a quarter-tick clock** on the set of shortest loops: exact along 98% of its
transitions and never over, short only where a faster lane merges. `EDGES.md` §5.

**the set of all shortest loops / `P*`** — `P_N` being every state on a loop of length `N` or
less, `P*` is `P_N` at the least `N` whose projection holds an 8-connected loop round the route.
The user's answer to "the shortest phase-complete path": the whole set, the shortest rather than
